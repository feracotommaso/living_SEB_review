```{r}
stop("As of apaquarto version 3.0.0, the 'include' line below the metadata is no longer used and must be deleted. Right below your yaml metadata, delete this line:\n\n`{{< include _extensions/wjschne/apaquarto/_apa_title.qmd >}}`\n\nAlso, replace all non-standard prefixes like apafg- and apatb- with the standard Quarto prefixes (i.e., fig- and tbl-). Also change references to tables and figures to standard Quarto syntax. For example, `{apafg-myplot}` should now be `@fig-myplot`.")
```
...
# }
# 
# journalmode <- get_metadata()$format$`apaquarto-pdf`$documentmode
# 
# if (is.null(journalmode)) {
#   isjournalmode <- FALSE
#   knitr::opts_chunk$set(twocolumnmode = FALSE)
# } else {
#   isjournalmode <- journalmode == "jou"
#   if (isjournalmode) {
#     knitr::opts_chunk$set(twocolumnmode = TRUE)
#   } else {
#     knitr::opts_chunk$set(twocolumnmode = FALSE)
#   }
# }
...
#       fignote <- NULL
#     } else {
#       fignote <-  paste0("\n\n\\figurenote{\n\\QuartoMarkdownBase64{", openssl::base64_encode(options$`apa-note`), "}\n}\n\n")
#     }
#     
#     figure_enviornment <- "figure"
#     if (!is.null(options$`apa-twocolumn`)) {
#       figure_enviornment <- ifelse(options$`apa-twocolumn` == TRUE, "figure*", "figure")
#     }
# 
# 
#     output <- paste0("\n\n\\begin{", figure_enviornment, "}[h!]\n",
#                    "\\caption{\\QuartoMarkdownBase64{", openssl::base64_encode(options$`apa-cap`), "}}\n",
#                    "\\label{", options$label, "}\n",
#       "\\includegraphics[width=",figwidth,"]{",
#       filename,
#       "}\n",
#       fignote,
#       "\n\\end{", figure_enviornment, "}\n\n"
#       )
#     # message(output)
#     output
#     })
...
#     if (!is.null(options$`apa-twocolumn`)) {
#       table_environment <- ifelse(options$`apa-twocolumn` == TRUE, "table*", "table")
#     }
#           
#           pre <- ""
# 
...
#                           table_environment, 
#                           "}\n\\caption{\\QuartoMarkdownBase64{", 
#                           openssl::base64_encode(options$`apa-cap`),
#                           "}}\n\\label{",
#                           options$`label`,"}\n","```\n")
#           }
#           
...
#             if (!all(is.null(options$`apa-note`))) {
#               
#               columnmodepost <- ifelse(options$twocolumnmode | isjournalmode,
#                                        "\\vspace{.5em}",
#                                        "")
#               
#               post <- paste0(post, 
#                              columnmodepost,
#                              "\\tablenote{\\QuartoMarkdownBase64{", 
#                              openssl::base64_encode(options$`apa-note`), 
#                              "}}\n\n")
#             }
#             
# 
#             post <- paste0("```{=latex}\n", post, "\\end{", table_environment, "}\n```\n")
#           }
#         } else {
#           if (!all(is.null(options$`apa-note`))) {
#             post <- paste(
...
#     map(\(d) {
#       if ("roles" %in% colnames(d)) {
#         if (is.list(d |> pull(roles))) {
#           # pull(d, roles) |> unlist() |> print()
#           d <- mutate(d, role_name = map(roles, \(x) {
#             if (is.list(x)) names(x) else x
#           })) |> 
#             unnest(role_name) |> 
#             mutate(role_degree = map(roles, \(x) {
#               if (is.list(x)) x[[1]] else "lead"
#             })) |> 
#             unnest(role_degree) |> 
#             select(-roles) |> 
#             rename(roles = role_name)
#           }
#       }
#       d
#     }) |> 
...
#         author,
#         " ![Orcid ID Logo: A green circle with white letters ID](paper/_extensions/wjschne/apaquarto/ORCID-iD_icon-vector.svg){width=16px} https://orcid.org/",
#         orcid
#       )
#     ) %>%
#     pull(orcid_display) %>%
