...
1.abstracting includes the ryyan exported articles after deduplication
	...
	- Papers are screened (title + abstract) and initial selection is conducted
		...
		- 'include' when the paper is in topic but probably some SEB measurement or framework
		- 'include' when the paper is in topic and looks like they adopt the SEB measurement
...
3.meta_data includes all the final data that will be used for the meta-analysis and review
	...
	- 'matrices' includes all the correlation matrices obtained from the included papers. These are calculated by
		- PREFERABLE: Calculating the correlation matrices from the available data
	...
	- 'review_data' includes he basic_info data of all included papers, comprehending also those excluded from the meta-analysis.This is generated within '3.2cor_to_metadata.R'.
