...
## Document and Data info

::: {.callout}
**Document last update:** `r Sys.Date()`
...
release <- fromJSON(
  "https://api.github.com/repos/feracotommaso/living_SEB_review/releases/latest"
)

cat(
  paste0(
    "**Data last updated:** ", as.Date(release$published_at), 
    "\n\n",
...
**Living SEB app:** available [here](https://feracotommaso.github.io/living_SEB_review/shiny-app.html){target="_blank"}
...
ggplot(metaRes, aes(x = r, y = reorder(skill,length(pred_vars):1))) +
  ...
  annotate("text", 
           ...
           hjust = "center", size = 5) +
  coord_cartesian(xlim = c(min(metaRes$rcil) -.05,
                           max(metaRes$rciu) + .05))
...
Onother interesting question we may tackle using the *Living SEB Skills Project*'s database, is **whether SEB skills are important for academic achievement**.
...
All analyses can be reproduced in the Meta-analysis and MetaSEM modules of the [*Living SEB App*](https://feracotommaso.github.io/living_SEB_review/shiny-app.html){target="_blank"}. Users can select specific subsets of studies, download data and R code, and generate updated reports.
