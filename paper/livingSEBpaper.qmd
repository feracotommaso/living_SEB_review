...
All past and future procedures for this project are preregistered and publicly available on [GitHub](https://feracotommaso.github.io/living_SEB_review/preregistration/Preregistration_protocol_livingSEB.pdf) . The repository (<https://github.com/feracotommaso/living_SEB_review>) also hosts all materials, data, and R code used in this article. All materials are also available on Zenodo (XXXX), where snapshots of monthly Git releases will be identified with specific DOIs for future use (e.g., for studies that need to reference the exact version of the Living SEB Skills Project used in their analyses). Analyses will strictly follow the preregistration unless otherwise noted. The systematic search and review is conducted according to the PRISMA guidelines [@moherPreferredReportingItems2015].
...
At the time of writing, the systematic search identified `r totDownload` publications (Scopus = `r totSc`; WOS = `r totWs`). After removing duplicates, screening titles and abstracts, and reviewing full texts, `r nReviewPapers` studies were retained for the living review and `r nMetaPapers` studies met criteria for quantitative synthesis. These include a total of `r nMetaSamples` independent samples (see @fig-prisma), with sample sizes ranging from `r minSampleSize` to `r maxSampleSize` (median = `r medSampleSize`). Main characteristics of all studies included in the review are reported in @tbl-studies-summary. The dataset used corresponds to the latest release available at the time of writing (March 2026: xxxxxx).
